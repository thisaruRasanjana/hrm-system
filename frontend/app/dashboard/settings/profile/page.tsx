...
export default function ProfileSettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    // Only send editable fields — never email, employee_id, department
    const payload = {
  ...
  return (
    <div className="w-full max-h-full">
      ...
      <div className="space-y-6">
          ...
          {/* Form */}
          <form onSubmit={handleSave} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-5">
              ...
              <div>
                ...
                <input name="first_name" type="text" value={formData.first_name} onChange={handleChange}
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#f08a4b] bg-white transition" />
              </div>
              <div>
              ...
              {/* Read-only fields */}
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Employee ID <span className="text-gray-400 font-normal text-xs">(read-only)</span></label>
                <input type="text" value={formData.employee_id} readOnly
                  className="w-full border border-gray-100 rounded-xl p-3 text-sm text-gray-400 bg-gray-50 cursor-not-allowed" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Department <span className="text-gray-400 font-normal text-xs">(read-only)</span></label>
                <input type="text" value={formData.department} readOnly
                  className="w-full border border-gray-100 rounded-xl p-3 text-sm text-gray-400 bg-gray-50 cursor-not-allowed" />
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Email Address <span className="text-gray-400 font-normal text-xs">(read-only)</span></label>
                <input type="email" value={formData.email} readOnly
                  className="w-full border border-gray-100 rounded-xl p-3 text-sm text-gray-400 bg-gray-50 cursor-not-allowed" />
              </div>
              <div>
                <label className="block text-sm font-bold text-gray-700 mb-2">Phone Number</label>
                <input name="phone_number" type="text" value={formData.phone_number} onChange={handleChange} placeholder="+1 (555) 000-0000"
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#f08a4b] bg-white transition" />
              </div>

              <div className="col-span-2">
                <label className="block text-sm font-bold text-gray-700 mb-2">Home Address</label>
                <input name="address" type="text" value={formData.address} onChange={handleChange} placeholder="Enter your full address"
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#f08a4b] bg-white transition" />
              </div>
              ...
              <div>
                ...
                <input name="emergency_contact_number" type="text" value={formData.emergency_contact_number} onChange={handleChange} placeholder="Emergency phone number"
                  className="w-full border border-gray-200 rounded-xl p-3 text-sm focus:outline-none focus:border-[#f08a4b] bg-white transition" />
              </div>
            </div>
