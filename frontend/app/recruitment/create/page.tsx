            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              ...
              <div data-field-error={errors.department ? true : undefined}>
                ...
                <input
                  ...
                  className={fieldCls(errors.department)}
                ...
                <div className="grid grid-cols-4 gap-3">
                  {(["Intern", "Junior", "Mid", "Senior"] as const).map((level) => (
                    <button
                      ...
                      key={level}
                      onClick={() => {
                        setForm((prev) => ({ ...prev, experience_level: level }));
                        setTouched((prev) => new Set(prev).add("experience_level"));
                      }}
                      className={`py-3 rounded-xl border text-sm font-medium transition ${form.experience_level === level
                          ? "bg-orange-400 text-white border-orange-400 shadow-sm"
                          : errors.experience_level
                            ? "bg-white text-red-400 border-red-300 hover:border-orange-300"
              ...
              <div className="border border-gray-200 rounded-xl overflow-hidden">
                <button
                  ...
                  className="w-full flex justify-between items-center px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
                >
                ...
                {panelOpen && (
                  <div className="p-6 border-t border-gray-200 space-y-4 bg-gray-50/50">
                    <div>
                      <label className="block mb-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Panel Head</label>
                      <EmployeeSearch users={users} onSelect={(id) => setPanel({ ...panel, panel_head_id: id })} placeholder="Search by name" />
                    </div>
                    <div>
                      <label className="block mb-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Panel Member 1</label>
                      <EmployeeSearch users={users} onSelect={(id) => setPanel({ ...panel, panel_member_1_id: id })} placeholder="Search by name" />
                    </div>
                    <div>
                      <label className="block mb-1.5 text-xs font-medium text-gray-500 uppercase tracking-wide">Panel Member 2</label>
                      <EmployeeSearch users={users} onSelect={(id) => setPanel({ ...panel, panel_member_2_id: id })} placeholder="Search by name" />
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setPanel({ ...panel, interview_link: e.target.value })}
                      />
                    </div>
                  </div>
                )}
              ...
              <div className="flex gap-4 pt-2">
                <button
                  ...
                  className="px-8 py-3 border border-orange-400 text-orange-500 rounded-xl hover:bg-orange-50 transition text-sm font-medium"
                >
                  Cancel
                </button>
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-8 py-3 bg-orange-400 hover:bg-orange-500 disabled:bg-orange-300 disabled:cursor-not-allowed text-white rounded-xl transition text-sm font-medium flex items-center gap-2"
                >
                  {submitting ? (
                    <>
