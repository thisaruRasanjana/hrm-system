  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/recruitment/candidates/${candidateId}`)
      ...
      .then((cand) => {
        ...
        return fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/recruitment/applications/${appId}/final-decision-view`);
      })
      .then((r) => r.json())
  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000'}/recruitment/applications/${data.application_id}/final-decision`,
        {
          method: "POST",
