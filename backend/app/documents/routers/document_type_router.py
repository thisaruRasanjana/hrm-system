...
def list_all_document_types(db: Session = Depends(get_db)): # HR only
    return document_type_service.list_all_document_types(db)


@router.get("/active/", response_model=list[DocumentTypeResponse])
def list_active_document_types(db: Session = Depends(get_db)): # Anyone can see active types
    return document_type_service.list_active_document_types(db)


@router.patch("/{type_id}", response_model=DocumentTypeResponse)
def update_document_type(type_id: UUID, data: DocumentTypeUpdate, db: Session = Depends(get_db)):
