...
@router.get("/", response_model=List[schemas.RoleOut], summary="List all roles")
def list_roles(db: Session = Depends(get_db)):
    return service.get_all_roles(db)


@router.get("/{role_id}", response_model=schemas.RoleOut, summary="Get a single role")
def get_role(role_id: int, db: Session = Depends(get_db)):
    return service.get_role_by_id(db, role_id)


@router.post("/", response_model=schemas.RoleOut, status_code=status.HTTP_201_CREATED, summary="Create a new role")
def create_role(payload: schemas.RoleCreate, db: Session = Depends(get_db)):
    return service.create_role(db, payload)


@router.put("/{role_id}", response_model=schemas.RoleOut, summary="Update a role (partial)")
...
@router.post("/assign", summary="Assign roles to a user (replaces existing roles)")
