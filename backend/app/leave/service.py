...
def create_leave(db: Session, employee_id: int, data: LeaveRequestCreate):
    ...
    if leave_type and "medical" in leave_type.name.lower() and not data.attachment_urls:
        raise ValueError("Medical leave requires a supporting document")
...
def update_status(db: Session, request_id: int, status: str, approved_by: int | None = None, rejection_reason: str | None = None):
